...
def create_ensemble_model():
    ...
    effnet_output = GlobalAveragePooling2D()(effnet.output)
    effnet_output = Dropout(rate=0.5)(effnet_output)
    effnet_output = Dense(4, activation='softmax')(effnet_output)

    # Define DenseNet121 model
    densenet = DenseNet121(weights='imagenet', include_top=False, input_shape=(224, 224, 3))
    densenet_output = GlobalAveragePooling2D()(densenet.output)
    densenet_output = Dropout(rate=0.5)(densenet_output)
    densenet_output = Dense(4, activation='softmax')(densenet_output)

    # Add outputs of both models
    ensemble_output = Add()([effnet_output, densenet_output])

    # Define ensemble model with a single Dense layer for 4 classes
    ensemble_model = Model(inputs=[effnet.input, densenet.input], outputs=ensemble_output)

    # Compile the ensemble model
    opt = tf.keras.optimizers.Adam(learning_rate=0.0001)
    ensemble_model_output = Dense(4, activation='softmax')(ensemble_output)
    ensemble_model = Model(inputs=[effnet.input, densenet.input], outputs=ensemble_model_output)
    ensemble_model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])
    
    return ensemble_model

# Create and print the ensemble model
...
def func_cv(X_train, y_train, cv_itr, n_cv):
    ...
    output_dir = "10_FOLD_Ensemble_EfficientNet_DenseNet121_Model"
    ...
    for train_ix, test_ix in cv_itr.split(X_train, y_train):
        ...
        with strategy.scope():
            ...
            history = model.fit([augmented_X_tr, augmented_X_tr],augmented_y_tr, validation_data=([x_ts, x_ts], y_ts), 
                                epochs =30, verbose=1, batch_size=32) #callbacks=[tensorboard,checkpoint,reduce_lr]
        ...
        plt.savefig(os.path.join(output_dir, f'Comfusion_matrix_10_FOLD_Ensemble_EfficientNet_DenseNet121_Fold_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Training_curve_10_FOLD_Ensemble_EfficientNet_DenseNet121_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Loss_curve_10_FOLD_Ensemble_EfficientNet_DenseNet121_Fold_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        plt.legend(loc="lower right", fontsize = 14)
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Receiver Operating Characteristic_10_FOLD_Ensemble_EfficientNet_DenseNet121_Fold_{ix}.pdf'))
        plt.show()
    ...
    plt.show()
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, 'Aggregated_Comfusion_matrix_10_Fold_CV_DCTN_VGG19.pdf'))
    #####################################################################################################
    ...
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, f'Aggregated_Training_curve_10_Fold_CV_DCTN_VGG19.pdf'))
    plt.show()
    ...
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, f'Aggregated_Loss_curve_10_Fold_CV_DCTN_VGG19.pdf'))
    plt.show()
    #####################################################################################################
    
    # SAVE MODEL
    model.save_weights("10_FOLD_EDNet.h5") 
...
print('*'*80)
print('*'*80)
print('Ensemble_EfficientNet_DenseNet121_MODEL')
print(time.strftime('%X %x %Z'))
print('*'*80)
print('*'*80)
