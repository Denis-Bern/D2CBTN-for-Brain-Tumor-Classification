...
def get_resnet152v2(input_shape, learning_rate):
    ...
    resnet_152_v2_model = ResNet152V2(include_top=False, weights='imagenet', input_shape=(input_shape, input_shape, 3))
...
model = get_resnet152v2(224, 0.0001)
model.summary() 
...
def func_cv(X_train, y_train, cv_itr, n_cv):
    ...
    output_dir = "10_FOLD_ResNet152_FINAL_Model"
    ...
    for train_ix, test_ix in cv_itr.split(X_train, y_train):
        ...
        with strategy.scope():
            ...
            history = model.fit(augmented_X_tr,augmented_y_tr, validation_data=(x_ts, y_ts), 
                                epochs =40, verbose=1, batch_size=32) #callbacks=[tensorboard,checkpoint,reduce_lr]
        ...
        plt.savefig(os.path.join(output_dir, f'Comfusion_matrix_10_FOLD_ResNet152_FINAL_Fold_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Training_curve_10_FOLD_ResNet152_FINAL_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Loss_curve_10_FOLD_ResNet152_FINAL_Fold_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        plt.legend(loc="lower right", fontsize = 14)
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Receiver Operating Characteristic_10_FOLD_ResNet152_FINAL_Fold_{ix}.pdf'))
        plt.show()
    ...
    plt.show()
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, 'Aggregated_Comfusion_matrix_10_Fold_ResNet152_FINAL.pdf'))
    #####################################################################################################
    ...
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, f'Aggregated_Training_curve_10_Fold_ResNet152_FINAL.pdf'))
    plt.show()
    ...
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, f'Aggregated_Loss_curve_10_Fold_ResNet152_FINAL.pdf'))
    plt.show()
    #####################################################################################################
    
    # SAVE MODEL
    model.save_weights("10_FOLD_ResNet152.h5") 
