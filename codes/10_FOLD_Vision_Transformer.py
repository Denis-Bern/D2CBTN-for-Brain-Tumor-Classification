...
# Plot the third row from the patched image
plt.figure(figsize=(10, 10))
n = int(np.sqrt(patches.shape[1]))
row = 3  # Row to plot
for i in range(n):
    patch_idx = n * row + i
    patch = patches[0, patch_idx]
    patch_img = tf.reshape(patch, (patch_size, patch_size, 3))
...
def func_cv(X_train, y_train, cv_itr, n_cv):
    ...
    output_dir = "10_FOLD_Vision_Transformer_Model"
    ...
    for train_ix, test_ix in cv_itr.split(X_train, y_train):
        ...
        with strategy.scope():
            ...
            history = model.fit(augmented_X_tr,augmented_y_tr, validation_data=(x_ts, y_ts), 
                                epochs = 100, verbose=1, batch_size=32) #callbacks=[tensorboard,checkpoint,reduce_lr]
        ...
        plt.savefig(os.path.join(output_dir, f'Comfusion_matrix_10_FOLD_Vision_Transformer_Fold_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        plt.xticks(range(0, len(epochs), 9),fontsize=18)  # Set x-axis ticks at intervals of 5
        ...
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Training_curve_10_FOLD_Vision_Transformer_Fold_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Loss_curve_10_FOLD_Vision_Transformer_Fold_{ix}.pdf'))
        plt.show()
        print('\n\n')
        ...
        plt.legend(loc="lower right", fontsize = 14)
        # Save the plot as a PDF file
        plt.savefig(os.path.join(output_dir, f'Receiver Operating Characteristic_10_FOLD_Vision_Transformer_Fold_{ix}.pdf'))
        plt.show()
    ...
    plt.show()
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, 'Aggregated_Comfusion_matrix_10_FOLD_Vision_Transformer.pdf'))
    #####################################################################################################
    ...
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, f'Aggregated_Training_curve_10_FOLD_Vision_Transformer.pdf'))
    plt.show()
    ...
    # Save the plot as a PDF file
    plt.savefig(os.path.join(output_dir, f'Aggregated_Loss_curve_10_FOLD_Vision_Transformer.pdf'))
    plt.show()
    #####################################################################################################
    
    # SAVE MODEL
    model.save_weights("10_FOLD_Vision_Transformer.h5")  
